...
class BaiduImageSpider(object):
    def __init__(self):
        ...
        self.directory = r"C:\\Users\\Administrator\\Desktop\\车表\\货车车系\\{}"  # 存储目录  这里需要修改为自己希望保存的目录  {}不要丢
        self.header = {
    ...
    def run(self, searchName):
        ...
        for index in range(self.json_count):
            ...
            for link in list_image_link:
                pic_number += 1
                py = p.get_pinyin(searchName, '')  # 转拼音
                pic_number2 = py + str(pic_number)
                self.save_image(link, self.directory.format(pic_number2 + '.jpg'))
                # time.sleep(0.2)  # 休眠0.2秒，防止封ip
        print(searchName + "----图像下载完成--------->")

    # 入口函数 搜索名 保存序号
    def run2(self, searchName, pic_number):
        # searchName = input("查询内容：")
        searchName_parse = parse.quote(searchName)  # 编码
        self.create_directory(searchName)
        for index in range(self.json_count):
...
if __name__ == '__main__':
    ...
    spider.json_count = 5  # 定义下载10组图像，也就是三百张
    spider.run("劲骐")
